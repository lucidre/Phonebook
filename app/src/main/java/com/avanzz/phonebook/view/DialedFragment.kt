...
class DialedFragment : Fragment() {

    private lateinit var dialedRecyclerView: RecyclerView
    ...
    companion object {
        fun newInstance(): DialedFragment {
            return DialedFragment()
        }
    }
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_dialed, container, false)

        dialedRecyclerView = view.findViewById(R.id.dialedRecyclerView)
        dialedRecyclerView.layoutManager = LinearLayoutManager(activity)
        //adapter = CallListAdapter()
        //receivedRecyclerView.adapter = adapter
        progress = ProgressDialog(activity)

        LoadCallListTask().execute(CallType.DIALED)
    ...
    private inner class LoadCallListTask : AsyncTask<CallType, Void, List<CallLogItem>>() {
        ...
        override fun onPostExecute(result: List<CallLogItem>) {
            ...
            dialedRecyclerView.adapter = adapter
        }
    }
